...
class AudioTranscriber:
    ...
    def process_audio(self, audio_file_path: str) -> List[Dict]:
        ...
        # ── GPU Usage print ──────────────────────────────────────────────────
        timings["peak_vram_gb"] = round(torch.cuda.max_memory_allocated() / 1024**3, 2)
        gpu_usage = get_gpu_usage()
        if gpu_usage.get("available"):
            print(f"--- [GPU Usage] used={gpu_usage['used_gb']} GB | reserved={gpu_usage['reserved_gb']} GB | peak={timings['peak_vram_gb']} GB | total={gpu_usage['total_gb']} GB | usage={gpu_usage['percent']}%")
    ...
    def _save_log(self, audio_file_path: str, duration_sec: float, duration_min: float, timings: dict, segments: List[Dict]):
        ...
        with open(log_path, "w", encoding="utf-8") as f:
            ...
            gpu_usage = get_gpu_usage()
            if gpu_usage.get("available"):
                f.write(f"  vram_used        : {gpu_usage['used_gb']} GB\n")
                f.write(f"  vram_reserved    : {gpu_usage['reserved_gb']} GB\n")
                f.write(f"  vram_total       : {gpu_usage['total_gb']} GB\n")
                f.write(f"  vram_usage_%     : {gpu_usage['percent']}%\n")
            else:
                f.write("  GPU not available\n")

            f.write("\n[GPU Peak Memory]\n")
            f.write(f"  # Peak VRAM recorded during the full pipeline run\n")
            f.write(f"  # Reset before audio load, captured after pipeline completes\n")
            if torch.cuda.is_available():
                f.write(f"  vram_peak        : {timings.get('peak_vram_gb', 'N/A')} GB\n")
            else:
                f.write("  GPU not available\n")
